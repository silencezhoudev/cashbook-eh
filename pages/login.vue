...
// 设置页面head
useHead({
  title: SystemConfig.value?.title || 'Cashbook',
  meta: [
    { name: 'description', content: SystemConfig.value?.description || 'Cashbook, 快速记账!' },
    { name: 'keywords', content: `Cashbook,记账本,私人记账,开源账本,dingdangdog,月上老狗,${SystemConfig.value?.keywords || ''}` }
  ]
});
...
const login = async () => {
  if (validateLoginForm()) {
    try {
      console.log("开始登录，用户名:", loginParam.value.username);
      const res = await doApi.post("api/login", loginParam.value);
      console.log("登录API响应:", res);
      Alert.success("登录成功");
      
      // 检查cookie是否设置成功
      const authCookie = useCookie("Authorization").value;
      console.log("登录后Authorization cookie:", authCookie ? authCookie.substring(0, 20) + "..." : "null");
      
      // 直接使用navigateTo，但先等待一下让cookie生效
      await new Promise(resolve => setTimeout(resolve, 500));
      
      console.log("准备跳转，fromUrl:", fromUrl.value);
      if (fromUrl.value) {
        console.log("跳转到:", fromUrl.value);
        await navigateTo(fromUrl.value);
      } else {
        console.log("跳转到首页");
        await navigateTo("/");
      }
...
const toGithub = () => {
  window.open("https://github.com/silencezhoudev/cashbook-eh", "_blank");
};
