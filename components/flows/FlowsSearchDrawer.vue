  ...
  >
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <!-- 借贷类型 -->
        <div v-if="localQuery.flowType === '借贷'" class="space-y-2">
          <label
            class="block text-sm font-semibold text-gray-600 dark:text-gray-400"
          >
            借贷类型
        ...
        <div class="space-y-2">
          ...
          <select
            v-model="localQuery.eliminate"
            class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-green-950 dark:text-white focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500"
          >
            <option value="">全部</option>
            <option value="0">计入流水</option>
            <option value="1">不计入流水</option>
          </select>
        </div>
...
const onFlowTypeChange = () => {
  ...
  // 如果不是借贷类型，清空借贷类型字段
  if (localQuery.value.flowType !== '借贷') {
    localQuery.value.loanType = "";
  }
};
