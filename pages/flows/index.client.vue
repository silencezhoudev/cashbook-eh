<template>
  <div class="p-2 md:p-4 bg-gray-50 dark:bg-green-950/20 min-h-full">
    ...
    <FlowsToolbar
      ...
      @batch-move-to-book="openBatchMoveDialog"
      @open-search="searchDrawer = true"
      @reset-query="resetQuery"
      @keyword-search="onKeywordSearch"
    ...
    <FlowsTable
      ...
      :page-size="flowQuery.pageSize || 100"
    ...
    <!-- 挖财导入对话框 -->
    <div
      v-if="showWacaiImportDialog"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"
      @click="closeWacaiImport"
    >
      <div
        class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col"
        @click.stop
    ...
    >
      ...
      >
        <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-600 flex justify-between items-center">
          <h3 class="text-lg font-semibold text-green-950 dark:text-white">将选中流水移动到其他账本</h3>
          <button
            @click="closeBatchMoveDialog"
            class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 p-1 rounded transition-colors"
          >
        ...
        <div class="p-4 space-y-4">
          <div class="space-y-2">
            <label class="block text-sm font-medium text-gray-600 dark:text-gray-400">目标账本</label>
            <select
            ...
            >
              ...
              <option 
                ...
                :key="book.bookId" 
                :value="book.bookId"
                :disabled="book.bookId === currentBookId"
              >
                {{ book.bookName }}
              </option>
            </select>
            <p class="text-xs text-gray-500 dark:text-gray-400">支持移动所有类型的流水记录（收入、支出、不计收支、借贷、转账）到其他账本。仅修改账本归属，保持账户关联、借贷关系、余额变动等业务逻辑不变。</p>
          </div>
        </div>
        ...
        <div class="px-4 py-3 border-t border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/30">
          <div class="flex gap-2">
            ...
            <button
              ...
              class="flex-1 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-sm font-medium transition-colors"
            >
              确认移动
            </button>
          </div>
...
const showWacaiImportDialog = ref(false);
...
const fetchAvailableBooks = async () => {
  try {
    const res = await doApi.post<any[]>("api/entry/book/page", {
      pageNum: 1,
      pageSize: 100,
    });
...
const doQuery = () => {
  ...
  const payload: any = { ...flowQuery.value };
  // 如果存在关键字搜索，则不传 bookId，实现跨账本搜索
  if (!payload.keyword || payload.keyword === "") {
    payload.bookId = localStorage.getItem("bookId");
  }
  doApi
    .post<Page<Flow>>("api/entry/flow/page", payload)
    .then((res) => {
      if (res) {
...
const totalPages = computed(() =>
  Math.ceil((flowPageRef.value?.total || 0) / (flowQuery.value.pageSize || 100))
);
...
const onKeywordSearch = (kw: string) => {
  flowQuery.value.keyword = kw || "";
  flowQuery.value.pageNum = 1;
  doQuery();
};

const onClearKeyword = () => {
  flowQuery.value.keyword = "";
  flowQuery.value.pageNum = 1;
  doQuery();
};

// 列筛选事件处理
const onColumnFilterChange = (data: { columnKey: string; filter: { selectedValues: string[]; keywordFilter: string } }) => {
  console.log('列筛选变化:', data);
  // 这里可以添加额外的处理逻辑，比如保存筛选状态到本地存储
  // 由于筛选是在前端进行的，不需要重新查询后端
};

const resetQuery = () => {
  flowQuery.value = {
    pageNum: 1,
    pageSize: 200, // 重置时显示500条记录
    startDay: "",
    endDay: "",
    attribution: "",
    name: "",
...
const confirmBatchMove = () => {
  ...
  if (targetBookId.value === currentBookId) {
    Alert.error("目标账本与当前账本相同");
    return;
  }
  
  // 统计普通流水和转账记录数量
  const flowIds = (selectedFlows.value || []).filter((id: any) => typeof id === 'string' ? id.startsWith('flow_') : true);
  const transferIds = (selectedFlows.value || []).filter((id: any) => typeof id === 'string' ? id.startsWith('transfer_') : false);
  
  if (flowIds.length === 0 && transferIds.length === 0) {
    Alert.error("没有可移动的记录");
    return;
  }
  
  let confirmMessage = `将移动以下记录到选定账本：\n`;
  if (flowIds.length > 0) {
    confirmMessage += `• ${flowIds.length} 条流水记录（包括收入、支出、不计收支、借贷等）\n`;
  }
  if (transferIds.length > 0) {
    confirmMessage += `• ${transferIds.length} 条转账记录\n`;
  }
  confirmMessage += `\n注意：仅修改账本归属，账户关联、借贷关系、余额变动等业务逻辑保持不变。\n\n是否继续？`;
  ...
  Confirm.open({
    ...
    confirm: () => {
      doApi
        .post("api/entry/flow/moveToBook", {
          ids: selectedFlows.value,
          fromBookId: currentBookId,
          toBookId: targetBookId.value,
        })
        .then(() => {
          Alert.success("移动成功");
...
const editItem = (item: any) => {
  ...
  // 传递原始流水数据，保证包含 accountId 等字段
  selectedFlow.value = item.originalFlow ? item.originalFlow : item;
  showFlowEditDialog.value = true;
};

// 编辑转账
const editTransfer = (item: any) => {
  // 复用流水编辑对话框，保持与新增“选择转账”后的表单一致
  dialogFormTitle.value = formTitle[1];
  selectedFlow.value = {
    id: item.originalFlow?.id || undefined, // 占位，无实际用于转账更新
    flowType: '转账',
    day: item.day,
    money: item.money,
    name: item.name,
    ...
    transferId: item.originalTransfer?.id,
    originalTransfer: item.originalTransfer,
  } as any;
  showFlowEditDialog.value = true;
};
...
const deleteItem = (item: any) => {
  ...
  const itemType = item.type === 'transfer' ? '转账' : '流水';
  const apiEndpoint = item.type === 'transfer' ? 'api/entry/transfer/del' : 'api/entry/flow/del';
  
  Confirm.open({
    title: "删除确认",
    content: `确定删除${itemType} 【${item.name}:${item.money}】吗?`,
    confirm: () => {
      doApi
        .post(apiEndpoint, {
          id: item.type === 'transfer' ? item.originalTransfer.id : item.originalFlow.id,
          bookId: localStorage.getItem("bookId"),
        })
        .then(() => {
          Alert.success("删除成功");
...
const readCsvInfo = (event: Event) => {
  ...
  // 检查文件类型是否有效（必须是支付宝、微信、京东或模板）
  if (fileType.value === "none") {
    Alert.warning("请先选择导入类型");
    // 清空文件输入框
    if (target) {
      target.value = "";
    }
...
const removeFile = () => {
  ...
  csvHeaders.value = {};
  csvDatas.value = [];
  csvFile.value = undefined; // 清除选中的文件
  // 清空文件输入框的值，避免重复触发
  if (csvFileInput.value) {
...
const closeWacaiImport = () => {
  showWacaiImportDialog.value = false;
};

const openWacaiImport = () => {
  showWacaiImportDialog.value = true;
  importDrawer.value = false;
};
