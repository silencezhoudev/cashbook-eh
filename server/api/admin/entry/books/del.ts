...
export default defineEventHandler(async (event) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    console.log(`[管理员] 开始删除账本 ${bookId} (ID: ${id})，用户ID: ${userId}`);
    ...
    console.log(`[管理员] 收集到 ${affectedAccountIds.size} 个受影响账户，${transferIds.size} 个转账记录`);
    ...
    console.log(`[管理员] 删除流水记录: ${deletedFlows.count} 条`);
    ...
    if (transferIds.size > 0) {
      ...
      console.log(`[管理员] 删除转账记录: ${deletedTransfers} 条`);
    }
    ...
    console.log(`[管理员] 删除固定流水记录: ${deletedFixedFlows.count} 条`);

    // 2.4 删除预算记录
    const deletedBudgets = await tx.budget.deleteMany({
      where: { bookId, userId }
    });
    console.log(`[管理员] 删除预算记录: ${deletedBudgets.count} 条`);

    // 2.5 删除类型关系记录
    const deletedTypeRelations = await tx.typeRelation.deleteMany({
      where: { bookId, userId }
    });
    console.log(`[管理员] 删除类型关系记录: ${deletedTypeRelations.count} 条`);

    // 2.6 删除应收款记录
    const deletedReceivables = await tx.receivable.deleteMany({
      where: { bookId, userId }
    });
    console.log(`[管理员] 删除应收款记录: ${deletedReceivables.count} 条`);

    // 3. 删除账本
    const deletedBook = await tx.book.delete({
      where: { id }
    });
    console.log(`[管理员] 删除账本: ${deletedBook.bookName}`);
    ...
    for (const accountId of affectedAccountIds) {
      try {
        ...
        console.error(`[管理员] 更新账户 ${accountId} 余额失败:`, error);
        // 继续处理其他账户，不中断整个流程
      }
    }
    console.log(`[管理员] 更新账户余额: ${updatedAccounts} 个账户`);

    const endTime = Date.now();
    console.log(`[管理员] 账本删除完成，耗时: ${endTime - startTime}ms`);
