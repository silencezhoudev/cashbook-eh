...
// 判断是否在总账本模式页面（账本管理、数据分析、账户管理）
const isTotalBookModePage = computed(() => {
  return route.path === '/books' || route.path === '/analysis' || route.path === '/accounts' || route.path === '/calendar';
});
</script>
      ...
      <div class="flex justify-between h-12 md:h-16">
        ...
        <div class="flex items-center flex-1 justify-center">
          <div class="flex items-center space-x-2 md:space-x-4">
            ...
            >
              <span v-if="isTotalBookModePage" class="hidden md:block">总账本模式</span>
              <span v-else class="hidden md:block">当前账本：</span>{{ isTotalBookModePage ? '' : bookName }}
            </span>
            <button
              v-if="!isTotalBookModePage"
